...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    var transactions: MutableLiveData<RealmResults<Transaction>?> = MutableLiveData<RealmResults<Transaction>?>()
    var categoriesTransactions: MutableLiveData<RealmResults<Transaction>?> = MutableLiveData<RealmResults<Transaction>?>()
    ...
    fun addTransactions() {
        ...
        realm.copyToRealmOrUpdate(
            Transaction(Constants.INCOME, "Business", "Cash", "Some note here", Date(), 500, Date().time)
        )
        realm.copyToRealmOrUpdate(
            Transaction(Constants.EXPENSE, "Investment", "Bank", "Some note here", Date(), -900, Date().time)
        )
        realm.copyToRealmOrUpdate(
            Transaction(Constants.INCOME, "Rent", "Other", "Some note here", Date(), 500, Date().time)
        )
        realm.copyToRealmOrUpdate(
            Transaction(Constants.INCOME, "Business", "Card", "Some note here", Date(), 500, Date().time)
        )
        // some code here
        realm.commitTransaction()
    }
